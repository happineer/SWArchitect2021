...
class ArtifactManager:
    ...
    def provision(self, identifier):
        ...
        print("cp -rf %s ." % (" ".join(artifact_list)))
        ret = os.system("cp -rf %s ." % (" ".join(artifact_list)))


def current_path_check():
    return os.getcwd().split("/")[-1] == "LgFaceRecDemoTCP_Jetson_Nano"
...
def main():
    ...
    if not current_path_check():
        print("[Error] should run this script on 'LgFaceRecDemoTCP_Jetson_Nano' path")
        exit(-1)
